...
void setup(){
  ...
  TWBR=1000L;                     // a frequencia do i2c: 10000L = 10kHz
...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print("; volts: "); Serial.print(valorvolts, 5);
  ...
  Serial.print(buffer[5],HEX); Serial.print(" - "); // A
  Serial.println(buffer[6],HEX);                  // B
}
...
void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  ...
  buffer[5] = (int) valorvolts;
  buffer[6] = (int) ((valorvolts - buffer[5])*100.0);
...
void funcaoDadoRecebido(int x){    // funcao chamada ao receber um dado
  ...
  Serial.print("chegou x="); Serial.print(x); Serial.print(" byterec = "); Serial.println(dadoRecebido,HEX);
  if (x>1){
    for (int count = 1; count < x; count++) dadoRecebido = Wire.read(); 
  }
}
