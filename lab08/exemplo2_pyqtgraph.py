...
def taxa():
    ...
    while conexaoSerial.inWaiting() == 0:
        time.sleep(0.01)
    atraso_do_arduino = conexaoSerial.read()
    
    print(ord(atraso_do_arduino))
    textoatraso.setText(f'atraso = {ord(atraso_do_arduino)} ms')
...
textoatraso = pg.TextItem(text="", color=(255,255,0), anchor=(1,0))
p1.addItem(textoatraso)
textoatraso.setPos(npontos,5) # adiciona o texto na posicao (0,0) do grafico
...
botao3 = QtGui.QPushButton('Intervalo Medições')
proxy3.setWidget(botao3)
botao3.clicked.connect(taxa)
